...
import { getMessagePreview } from "../lib/media";
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 text-left min-w-0">
        ...
        <p className="text-xs text-base-content/70 truncate mt-0.5 flex items-center gap-1">
          {isTyping ? (
            "typing..."
          ) : (
            <>
              {(chat.lastMessage?.messageType === "image" || chat.lastMessage?.text === "Photo") && (
                <ImageIcon className="size-3 shrink-0" />
              )}
              <span className="truncate">{getMessagePreview(chat.lastMessage)}</span>
            </>
          )}
        </p>
      </div>
